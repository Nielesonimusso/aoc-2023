...
def stringDifference(a: str, b: str):
    return sum(int(x != y) for x, y in zip(a,b))


def reflectsColumn(pattern: list[str], column: int) -> bool:
    columns = ColumnView(pattern)
    return sum(stringDifference(columns[a], columns[b]) for a,b 
               in reflectedSequence(column, 0, len(columns))) == 1


def reflectsRow(pattern: list[str], row: int) -> bool:
     return sum(stringDifference(pattern[a], pattern[b]) for a,b 
               in reflectedSequence(row, 0, len(pattern))) == 1
